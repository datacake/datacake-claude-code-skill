...
Define downlinks on API products: `createApiDownlink(input: { productId, name, description, payloadEncoder, fieldsUsed: [], triggerOnMeasurements: false, kind: MQTT | HTTP })` or `createMqttDownlink(input: { product, name, description, payloadEncoder, fieldsUsed, triggerOnMeasurements })` (`devices` permission; `payloadEncoder` is the JavaScript `function Encoder(measurements, port) { … }`); update with `updateApiDownlink`/`updateMqttDownlink`, delete with `deleteApiDownlink(input: { id })` and `deleteMqttDownlink(input: { downlink })` (note the different key). `apiDownlinks` lists MQTT-kind downlinks too. Downlinks whose encoder reads fields take their input from the current field values; write them first (REST/`setValue`) or use the portal form. Public dashboards pass `publicDeviceAuth`/`publicDashboardAuth`. Legacy product functions: `callProductFunction(deviceId, functionId)`. Fleet-wide downlinks: a rule with a `MULTI_DEVICE_DOWNLINK` action.
