...
| Actions | `EMAIL`, `SMS` (needs credits), `WEBHOOK` (`POST` to any HTTP API such as Slack, Teams or a ticket system: static URL and headers, templated JSON body, request and response logged), `PUSH` (Datacake mobile app, workspace members), `SINGLE_DEVICE_DOWNLINK` (to the triggering device unless a device is given: use it for "every device gets its downlink"), `MULTI_DEVICE_DOWNLINK` (one trigger sends to every device of a product + tags; on a `DEVICE_LEVEL` rule of the same product this multiplies to devices × devices), `SET_VALUE` (write a field on the triggering or another device); each action fires when conditions become hot, stay hot and/or become cold, with `minSecondsBetweenHotConditions`, `maxConsecutiveActionExecutions` and weekday/time restrictions |
