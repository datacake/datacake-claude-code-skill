...
1. Clarify: what should trigger (measurement threshold, offline, schedule, zone), for which product and devices (all, tags, explicit), who gets notified how (email, SMS, push, or a webhook: a `POST` with static URL/headers and a JSON template body that reaches Slack, Teams, ticket systems and any REST API) or which downlink/set value runs, and whether reminders and an all-clear are wanted. For downlinks: "each device gets its own downlink" (e.g. all valves at 09:00) is a `DEVICE_LEVEL` rule with `SINGLE_DEVICE_DOWNLINK` and no device id, never `MULTI_DEVICE_DOWNLINK` (that multiplies devices × devices); `MULTI_DEVICE_DOWNLINK` is for one trigger hitting a whole (other) product.
